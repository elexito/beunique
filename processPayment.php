...
$_SESSION['phonenumber'] = $postData['phonenumber'];
$_SESSION['email'] = $postData['email'];
$_SESSION['session'] = $postData['session'];
}

$prefix = 'BeUnique Academy'; // Change this to the name of your business or app
$overrideRef = false;
...
//         $con = mysqli_connect("localhost","root","","beunique");
...
// mysqli_query($con,"INSERT INTO students (last_name, first_name, phone_number, email, session, lesson)
// VALUES ('j', 'j', 'k', 'k', 'i', '9')");
        ...
        if($transactionData->status === 'successful'){
          if($transactionData->currency == $_SESSION['currency'] && $transactionData->amount == $_SESSION['amount']){
              ...
              if($_SESSION['publicKey']){
                $con = mysqli_connect("localhost","root","","beunique");
                 $phonenumber = $_SESSION['phonenumber'];
            $quer = "SELECT * FROM students WHERE phone_number = '$phonenumber'";
$result = mysqli_query($con,$quer);
$row = $result->fetch_assoc();
$id = $row["id"];
            if($_SESSION['amount'] == "45000"){
    $pp = "WB";
}elseif ($_SESSION['amount'] == "40000") {
    $pp = "DM";
}elseif ($_SESSION['amount'] == "25000") {
    $pp = "GRA";
}
            $answer = "BU/".$pp."/".$_SESSION['session']."/".$id;
